...
Durum: **Tamamlandı (2026-09-06).** İlk tarama yalnız tespitti; tüm maddeler (87 uygulandı + 3 karar/koruma maddesi kapatıldı) çözüldü. Uygulanmış örnekler: `tasks/humanizer-changelog.md` §1–§20 + `tasks/humanizer-changelog-tefekkur.md`. Yeni içerik için kalıcı kural: CLAUDE.md §13.34.
...
- [x] **"Asıl soru / asıl mesele" (§27)** ✅ (2026-09-06: incelendi, KORUNDU — yazarın sesi): kaynak-yuzey + anlam-yaratilis-senteni (contrastDuo bridge'leri), rahmetin-grameri-4 + yaratilis-hikayesi-1 ("Belki de asıl soru şu:") gövde paragrafları. Bunlar makine artefaktı değil, yazarın doğal Türkçe retoriği; humanizer'ın "yazarın sesini eşle/koru" kuralı + §13.29 (Felsufi'ye sadakat) gereği dokunulmadı. Isimlendirme.jsx (UI katmanı, site dizesi) zaten düzeltilmişti (bkz. §6).
- [x] **EN çevirilerde AI kelimeleri** ✅ (2026-09-06: kelime avı tamamlandı, bkz. changelog §20): "A crucial qualifier" → "One important note" (lehv), "the crucial point is this" → "the important thing is this" (enerji-krizi), "underscores this fundamental role" → "points to this basic role too" (terminoloji-3-fizikalizm), "fundamentally how human comprehension works" → "basically how human comprehension works" (vicdan-evrensel-tercuman). "The unique role is never discovered" bulunamadı (EsmaTanimlari önceki turda düzeltildi, §6). Her biri TR karşılığına sadık kalınarak sadeleştirildi; TR'de bu kelimeler yoktu, yani makine-çeviri artefaktıydı. **Follow-on:** §13.22 gereği bu 4 EN gövde dizesi corpus-raw/embeddings'te de geçiyor — tam sadakat için `npm run embed:corpus` + `node scripts/build-embeddings.mjs` (incremental) çalıştırılabilir; değişiklik minimal EN-gövde olduğu için concierge sonuçlarına etkisi ihmal edilebilir, API maliyeti nedeniyle kullanıcı onayına bırakıldı.
- [x] **Bilimsel ad-anma (§2/§5)** ✅ (2026-09-06: fMRI cümlesi Newberg 2015 + Hölzel 2010 ile yeniden yazıldı, desteklenmeyen "prefrontal artar" çıkarıldı) (2026-09-06: "doğrular/confirms" → "bulur/finds" yapıldı; "fMRI çalışmaları gösteriyor" atıfsız kaldı, kaynak eklenmesi yazar kararı): "Nörobilim (Schubert 2005, embodied cognition) bunu doğrular", "Termodinamiğin ikonu Rod Swenson", "fMRI studies show" (en.json psychology). Kaynak varsa tamam; "doğrular" fiili §13.24 kuralına aykırı (bilim tasdik etmez).
...
- [x] Katalog açıklamaları "N · N · N" etiket formatında ✅ (2026-09-06: §9 kararıyla zaten uygulandı; doğrulandı). Karar: bu format yalnız katalog/menü/InventoryStrip kartlarında kalır (kabul edilen kart bağlamı), cümle beklenen yerlerden (DESC, kicker cümlesi, tldr, blurb) çıkarıldı. Doğrulama: `toolCatalog.js` desc'lerde 0 `·`, `page.js` DESC'lerinde `·` yok; homeCards kicker'ları stat etiketi olarak korunuyor (§2 kararı: slogan çıktı, sayılar kaldı).

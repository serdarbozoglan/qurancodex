...
### Faz 7.5 Açık
- [~] **Per-prophet OG image** — `docs/reviews/2026-05-26-per-prophet-og-image-audit.md`. **Fizibilite audit + tasarım + skeleton**. Veri hazır (kissa-atlas.json'da 4 peygamber: musa, yusuf, ibrahim, isa — her birinde color + nameAr/Tr/En + scenes). Route iskeleti `/atlas/peygamber/page.js` mevcut; `[id]` alt route deferred. Pre-generate: 4 × 2 locale = 8 PNG (~3.2 MB). Implementation deferred (route + ImageResponse + KFGQPC lazy-load).
...
- [x] **W22-U2** Browser back audit + HIGH/MED fixes — `docs/reviews/2026-05-25-browser-back-audit.md`. **2 HIGH FIXED** (ReadingModeRoute router.back, useQuranNav ?verse= align). **MED PARTIAL**: 5 component'ta (KiyametSahneleri, SunnetullahAtlasi, NefisMertebeleri, ZamanBoyutlari, Melekler) `dispatchEvent('openX')` → `useQuranNav.openOverlay(key)` migrate edildi (15 unique cross-tool nav action). Navbar.jsx 2 dead listener temizlendi. MeselAtlasi + sections/* + data/paths.jsx kalan event-dispatch'ler sonraki tur.
- [x] **W22-U3** Keyboard focus trap (audit + hook + entegrasyon) — `docs/reviews/2026-05-25-keyboard-focus-trap-audit.md`. **`useFocusTrap()` reusable hook** eklendi (`hooks/useFocusTrap.js`, 107 satır): trigger element capture, Tab/Shift+Tab boundary loop, unmount'ta focus return. **2 modal entegrasyonu**: TafsirPanel + WordPopover. WCAG 2.1 SC 2.4.3 compliance. Diğer modal'lara yayma sonraki tur (12+ component).
